...
public/           verbatim-copied to dist/: index assets (icon.svg, site.webmanifest), legal (privacy.html, terms.html, impressum.html, widerrufsbelehrung.html), 404.html, robots.txt, sitemap.xml, _headers, theme-init.js (FOUC bootstrap), auth/callback.html + callback.js (OAuth/magic-link landing). The four legal pages cross-link via a `.legal-footer` block. Operator details are now filled in (Abdurahim Hudulov, Silcherstraße 72, 74074 Heilbronn, Freiberufler, USt-IdNr DE461410996, supervisory authority LfDI BW) — if any of those change, search for the literal strings across `public/*.html` and update consistently.
