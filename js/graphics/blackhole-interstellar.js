...
// Recreates the iconic accretion disk with proper light bending and Doppler beaming
// Enhanced v2.0 with relativistic effects
...
function createAccretionDisk() {
    ...
    // Shader with gravitational lensing AND Doppler beaming
    const diskMaterial = new THREE.ShaderMaterial({
        ...
        vertexShader: `
            ...
            void main() {
                ...
                vec3 worldVelDir = normalize((modelMatrix * vec4(velocityDir, 0.0)).xyz);
                vec3 toCamera = normalize(cameraPosition - lensedWorldPos);
                float dopplerCos = dot(worldVelDir, toCamera);
                float v = orbitalSpeed * 0.15;
                vDopplerFactor = pow(1.0 + dopplerCos * v, 3.0);
                ...
                float dopplerSize = mix(0.85, 1.15, (vDopplerFactor - 0.7) / 0.6);
                gl_PointSize = size * pixelRatio * distanceFactor * turbulence * dopplerSize;
        ...
        fragmentShader: `
            ...
            void main() {
                ...
                float dopplerStrength = (vDopplerFactor - 1.0) * 2.0;
                if (dopplerStrength > 0.0) {
                    dopplerColor.r *= 1.0 + dopplerStrength * 0.1;
                    dopplerColor.g *= 1.0 + dopplerStrength * 0.2;
                } else {
                    dopplerColor.r *= 1.0 - dopplerStrength * 0.1;
                    dopplerColor.b *= 1.0 + abs(dopplerStrength) * 0.15;
                }

                vec3 baseColor = dopplerColor * radialBrightness;
                ...
                float dopplerBrightness = clamp(vDopplerFactor, 0.5, 1.8);
                float alpha = intensity * vIntensity * softBlend * radialBrightness * lensingBoost * dopplerBrightness;
                alpha = pow(alpha, 0.85 + distanceSoftness * 0.15);

                vec3 finalColor = mix(baseColor, glowColor, intensity * 0.7) * dopplerBrightness;
...
export function cleanupBlackHole() {
    ...
    if (einsteinRing) {
        ...
        if (sceneRef) sceneRef.remove(einsteinRing);
        einsteinRing = null;
    }

    if (volumetricGlowMesh) {
        volumetricGlowMesh.geometry.dispose();
        volumetricGlowMesh.material.dispose();
        if (sceneRef) sceneRef.remove(volumetricGlowMesh);
        volumetricGlowMesh = null;
    }
}
