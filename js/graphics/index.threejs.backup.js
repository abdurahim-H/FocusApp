...
export function init3D() {
    ...
    try {
        ...
        // Initialize camera effects with camera reference
...
function animate() {
    ...
    // Update star field - visible rotation
    if (starField) {
        starField.material.uniforms.time.value = elapsed;
        starField.rotation.y += 0.001;  // 20x faster rotation
    }
    ...
    // DRAMATIC camera motion - fast orbit around black hole
    const radius = 40 + Math.sin(elapsed * 0.2) * 15;  // 4x faster variation, bigger range
    const height = 15 + Math.sin(elapsed * 0.3) * 12;  // 3x faster, more vertical movement
    camera.position.x = Math.sin(elapsed * 0.15) * radius;  // 5x faster orbit
    camera.position.y = height;
    camera.position.z = Math.cos(elapsed * 0.15) * radius;
    camera.lookAt(0, 0, 0);
