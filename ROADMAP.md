...
- [x] **4.6 Theme: Aurora Plain (XL).** Six-module composition under `js/graphics/aurora/`: domed sky with stars + low moon glow (`aurora-sky`), wraparound aurora belt of six broadside planes carrying eight discrete curtain tongues with multi-octave horizontal sway + vertical ray streaks + slowly cycling palette (green→teal→cyan→violet→magenta) (`aurora-curtains`), twelve-slice ringed mountain silhouettes with feathered edges and aurora-tinted backlight (`aurora-mountains`), a 280-radius icy plain disc with drifting fog + procedural ice cracks + horizon aurora reflection (`aurora-plain`), six-plane glow ring at the horizon (`aurora-glow`), and a 1400-particle GPU snow system (`aurora-snow`). All shader time wrapped at 4-hour cycle per CLAUDE.md. Registered in `theme-registry.js`; settings card added in `scene.theme`; `scene-manager.js` render loop now drives non-Black-Hole themes via `updateActiveTheme()`.
