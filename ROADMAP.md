...
- [~] **2.9 Project / tag grouping (L).** Data side: `project` field on the task model (32-char trimmed string), `setTaskProject(id, name)` mutation, inline `#project` chip rendered next to the task text when set. Still open: an editor UI to set the project from the row, a filter, and the per-project view in Profile → Tasks. (Will land alongside 2.11 task detail surface.)
