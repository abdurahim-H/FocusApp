...
### Added
- **Music services dock** at bottom-left — connect buttons for Spotify, YouTube Music, Apple Music, YouTube, and SoundCloud. Pro feature: free users land in the upgrade modal, signed-in Pro users see a "wiring in progress" toast until the OAuth client for each provider is registered (the dev-account setup that has to happen outside the codebase). Brand icons stay recognisable on hover and when connected; rest-state colour and dock chrome retune per theme.
